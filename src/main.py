...
def run_pipeline(project_root: Path) -> None:
    ...
    for xml_file in sorted(input_dir.glob("*.xml")):
        ...
        except Exception as exc:  # protects the whole batch from one malformed file
            rejected_records.append({
                "file_name": xml_file.name,
                "message_id": None,
    ...
    if valid_records:
        pd.DataFrame(valid_records).to_csv(output_dir / "perinatal_records.csv", index=False)
        export_fhir_bundles(valid_records[:3], output_dir / "fhir_bundle_examples.json")
    else:
        pd.DataFrame().to_csv(output_dir / "perinatal_records.csv", index=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run perinatal XML ETL pipeline")
